...
namespace StageSimWASAPI
{
    ...
    public class TempoTracker
    {
        ...
        // Actual frame interval — updated each call to RegisterOnsetStrength
        private float _actualFrameInterval = 1f / 50f;  // seed with 50fps assumption
        ...
        {
            // Track actual frame interval for correct BPM calculation
            if (_lastFrameTime > 0f)
            {
                float elapsed = time - _lastFrameTime;
                if (elapsed > 0.001f && elapsed < 1.0f)  // sanity check
                    _actualFrameInterval = _actualFrameInterval * 0.9f + elapsed * 0.1f;
            }
            _lastFrameTime = time;
        ...
        private void EstimateTempo()
        {
            ...
            int minLag = Mathf.RoundToInt((60f / MAX_BPM) / _actualFrameInterval);  // frames at max BPM
            int maxLag = Mathf.RoundToInt((60f / MIN_BPM) / _actualFrameInterval);  // frames at min BPM
            ...
            for (int lag = minLag; lag <= maxLag; lag++)
            {
                float bpm = 60f / (lag * _actualFrameInterval);
            ...
            if (bestLag > 0)
            {
                float estimatedPeriod = bestLag * _actualFrameInterval;
