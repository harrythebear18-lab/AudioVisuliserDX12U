// Mode 54: Cymatic Standing Waves — 3D Chladni patterns from audio
// Standing wave nodes form 3D Chladni patterns. Bands control wave modes.
// Phase coherence = pattern symmetry. LUFS = wave amplitude. Crest = node sharpness.
// THD = pattern noise. Beat = mode shift. Kick = amplitude spike. Transient = pattern break.
...
float cymaticField(float3 p, float bands[8], float lufs, float crest, float thd,
                   float beatPulse, float kickSurge)
{
    float phaseCoh = phaseCoherence();
    float mode1 = bands[0] * 2.0 + 1.0;
    float mode2 = bands[2] * 2.0 + 1.0;
    float mode3 = bands[4] * 1.5 + 0.5;

    float wave = sin(mode1 * p.x * PI) * sin(mode2 * p.z * PI);
    wave += sin(mode3 * p.y * PI) * cos(mode1 * p.x * PI) * 0.5;
    wave += sin(mode2 * p.y * PI) * sin(mode3 * p.z * PI) * 0.3;
    wave *= (1.0 + lufs * 0.3);
    wave += beatPulse * 0.2 * sin(p.x * 4.0 + p.z * 4.0 + Time * 2.0);
    wave += kickSurge * 0.3 * sin(length(p.xz) * 6.0 - Time * 3.0);
    wave += thd * 0.1 * (hash11(p.x * 30.0 + p.z * 25.0 + Time) - 0.5);
    wave += crest * 0.15 * sin(p.x * 8.0 + p.z * 6.0 + p.y * 4.0);

    float surfaceY = wave * 0.3;
    float d = p.y - surfaceY;

    float plateD = sdBox(p - float3(0, -0.05, 0), float3(2.5, 0.02, 2.5));
    d = min(d, plateD);
...
float4 main(float4 pos : SV_Position, float2 uv : TEXCOORD0) : SV_Target
{
    ...
    float camAng = a.section * 0.4 + a.stereoBal * 0.3 + panMod * 0.2 + Time * 0.03 * a.motSpeed;
    ...
    [loop] for (int i = 0; i < MARCH_STEPS; i++) {
        ...
        float d = cymaticField(sp, bands, lufs, crest, thd, beatPulse, kickSurge);
    ...
    if (hit) {
        ...
        float3 n = normalize(float3(
            cymaticField(hp + float3(eps, 0, 0), bands, lufs, crest, thd, beatPulse, kickSurge)
          - cymaticField(hp - float3(eps, 0, 0), bands, lufs, crest, thd, beatPulse, kickSurge),
            2.0 * eps,
            cymaticField(hp + float3(0, 0, eps), bands, lufs, crest, thd, beatPulse, kickSurge)
          - cymaticField(hp - float3(0, 0, eps), bands, lufs, crest, thd, beatPulse, kickSurge)
        ));
        ...
        float3 waveCol = hsv(a.hueBase + iterFrac * a.hueRange + hueShift, 0.6 * a.satur, 0.9);
        waveCol = lerp(waveCol, lerp(a.brainCol, a.brainCol2, iterFrac), 0.3);
        waveCol = lerp(waveCol, a.brainCol3, phaseCoh * 0.2 + kickSurge * 0.15);

        float edge = pow(1.0 - max(dot(n, vDir), 0.0), 2.0) * crest;

        float3 litCol = waveCol * (diff + diff2) * (0.3 + a.brightness * 0.3 + a.dynamic * 0.2);
        litCol += float3(0.9, 0.85, 0.8) * spec * (0.5 + a.dynLight * 0.7);
        litCol += waveCol * fres * (0.4 + envelope * 0.4 + a.glow * 0.2);
        litCol += a.brainCol3 * edge * 0.1;
        litCol += waveCol * beatPulse * 0.15 * silence;
        ...
        litCol += waveCol * a.punch * 0.05 * silence;
